...
final class WatchSyncStore: NSObject, ObservableObject {
    ...
    func activateSessionIfNeeded() {
        ...
        if let data = session.applicationContext[Keys.snapshotData] as? Data {
    ...
    func requestRefresh() async {
        ...
        await withCheckedContinuation { continuation in
            let message: [String: Any] = [Keys.requestType: Keys.refresh]
    ...
    private func restoreCachedSnapshot() {
        guard let data = defaults.data(forKey: Keys.cachedSnapshot) else { return }
        consumeSnapshotData(data)
    }
...
extension WatchSyncStore: WCSessionDelegate {
    ...
    nonisolated func session(_ session: WCSession, didReceiveApplicationContext applicationContext: [String : Any]) {
        guard let data = applicationContext[Keys.snapshotData] as? Data else { return }
        Task { @MainActor in
            self.consumeSnapshotData(data)
        }
    }
    ...
    nonisolated func session(_ session: WCSession, didReceiveUserInfo userInfo: [String : Any] = [:]) {
        guard let data = userInfo[Keys.snapshotData] as? Data else { return }
        Task { @MainActor in
            self.consumeSnapshotData(data)
        }
    }
